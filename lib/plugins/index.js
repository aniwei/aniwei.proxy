...
pluginsManager.json = function () {
  ...
  js = {
    listing: json.listing.map(function (plugin) {
      return plugin.json();
    }),
    table: json.table,
  };
...
pluginsManager.push = function (plugin) {
  ...
  if (plugin.path) {
    exist = fs.existsSync(path.join(plugin.path, 'ui/index.jsx'));
  }
...
function Plugin (name, pluginPath, version, on) {
  this.name         = name;
  ...
  Object.defineProperty(this, 'settings', {
    get: function () {
      ...
      filePath = path.join(constants.path, '.' + this.name + 'rc');

      if (!fs.existsSync(filePath)) {
        writer(filePath, this._settings = {});
      } else {
        this._settings = reader(filePath);
      }
...
Plugin.prototype = {
  ...
  save: function () {
    var filePath = path.join(constants.path, '.' + this.name + 'rc');
    writer(filePath, this._settings);
