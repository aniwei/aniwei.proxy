...
class Application {
  constructor (config) {
    ...
    this.proxyServer.on('request', (req, res) => this.app(req, res));
    this.proxyServer.on('connect', (...args) => water.run([...args, app], () => tunnel([...args, app])));
  ...
  listen (port, ...other) {
    ...
    this.proxyPort = port;
    this.proxyServer.listen.apply(this.proxyServer, other);
  }

  local (urlParsed) {
    const ip          = app.ip;
    const port        = parseInt(urlParsed.port);
    const proxyNumber = this.settings.get('proxy-port-number');
    const hostname    = urlParsed.hostname;

    return (
      hostname === this.settings.get('hostname') ||
...
function createApplication (config) {
  ...
  app.plugins(require('./plugins/plugin-open-router'));
