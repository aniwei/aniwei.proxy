var urlParser     = require('url'),
    http          = require('http'),
    https         = require('https'),
    zlib          = require('zlib'),
    express       = require('express'),
    _             = require('lodash'),
    fs            = require('fs'),
    path          = require('path'),
    async         = require('async'),
    proto


proto = module.exports = function (router, description) {
  var ui          = this.config('ui'),
      app         = require(ui.server)(this),
      configure   = description.configure,
      hosts       = ui.hostname,
      Tunnel      = this.Tunnel,
      parallel    = [],
      context     = this,
      httpConfig;

  this.on('proxy.request', function (req) {
    var proxy   = hosts.proxy,
        host    = req.hostname,
        project = hosts.project

    if (!(host == proxy || host == project)) {
      context.io.emit('proxy', {
        type: 'request',
        data: req
      })
    }
  });

  this.on('proxy.response', function (res) {
    var proxy   = hosts.proxy,
        host    = res.hostname,
        project = hosts.project

    if (!(host == proxy || host == project)) {
      context.io.emit('proxy', {
        type: 'response',
        data: res
      })
    }
  });

  parallel.push(function (done) {
    var project = hosts.project,
        proxy   = hosts.proxy,
        credent = context.credentials,
        cert    = credent.publicToPem(credent({}, proxy, project)),
        key     = credent.pemKey,
        server;

    context.beside(proxy);
    context.beside(project);

    server = createServer(https, app, {
      key: key,
      cert: cert
    });

    server.on('listening', function () {
      var addr    = server.address(),
          port    = addr.port,
          tunnel  = Tunnel.get(proxy, port);

      tunnel.tunnelConfig.hostname = '127.0.0.1';

      done();
    });
  })

  parallel.push(function (done) {
    var server = createServer(http, app);

    server.on('listening', function () {
      var addr = server.address();

      httpConfig = {
        port: addr.port,
        ip:   '127.0.0.1'
      }

      done()
    })
  })

  router.use(function(req, res, next){
    var proxy     = req.proxy,
        isHttp    = req.protocol == 'http';

    if (proxy.hostname == hosts.proxy) {
      if (!httpConfig) {
        return async.parallel(parallel, function (result) {
          _.assign(proxy, httpConfig);

          next();
        });
      } else {
        if (isHttp) {
          _.assign(proxy, httpConfig);
        }
      }
    }

    next()
  })
}

function createServer (client, app, argv) {
  var server = client.createServer.call(client, argv);

  server.setTimeout(0);
  server.listen(0);
  server.on('request', app);

  return server;
}

proto.description = {
  name:   'ui',
  text:   '试图界面',
  on:     true,
  uri:    __dirname
}
