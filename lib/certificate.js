...
module.exports = {
  ...
  save: function save() {
    ...
    this.save = function () {
      ...
      timer = setTimeout((function () {
        fs.writeFile(path, plist.build(table));
      }).bind(this), 200);
    }
  ...
  forge: function (model, altname) {
    ...
    data = this.toPem(this.create({
      attributes: [{ name: 'commonName',value: 'aniwei.tech'}].concat(config.attributes),
  ...
  create: function (options) {
    ...
    cert.setIssuer(options.subject);
  ...
  root: function () {
    var cert,
        config = configure.get('certificate-configure').value,
        attr      = [
          { name: 'commonName',value: 'aniwei.tech'}
